...
public class WindRPool<T> {
	...
	public void setConfig(RPConfig config) throws ResourceBuildException {
		this.config = config;
	...
	private Partition<T> createPartition(RPConfig config) throws ResourceBuildException {
		return new Partition<T>(config.getInitResourceCountPerPartition(),
	...
	public ResourceEntry<T> getResourceEntry() throws ResourceBuildException {
		...
		if(resourceEntry == null) {
			throw new ResourceBuildException("cannot find availiable resource");
		}
		return resourceEntry;
	}
